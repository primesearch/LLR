...
/* Instantiated by configure. */
#if ! defined (__GMP_WITHIN_CONFIGURE)
//#define _LONG_LONG_LIMB  1 // jpenne
...
typedef unsigned _int64	mp_limb_t;
typedef _int64		mp_limb_signed_t;
#else
...
mpz_get_ui (mpz_srcptr __gmp_z) __GMP_NOTHROW
{
  ...
  return (__gmp_n != 0 ? __gmp_l : 0); // jpenne
//  return ((unsigned __int64)__gmp_n != 0 ? (unsigned int)__gmp_l : 0);
#else
...
mpn_neg (mp_ptr __gmp_rp, mp_srcptr __gmp_up, mp_size_t __gmp_n)
{
  ...
  do {
      __gmp_ul = *__gmp_up++;
      *__gmp_rp++ = 0 -__gmp_ul - __gmp_cy; // jpenne
      __gmp_cy |= __gmp_ul != 0;
  } while (--__gmp_n != 0);
  return __gmp_cy;
}
