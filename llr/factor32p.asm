...
;
; Adapted from factor32.asm by Jean Penné, March 2006
; Email: jpenne@wanadoo.fr
; This routine implements fast trial factoring of 4^p+1 numbers, where p is prime.
; The goal is to select exponent candidates for Gaussian-Mersenne and Gaussian-Fermat(?) prime search.
;
; This only runs on 32-bit CPUs.
;
; Here are some precisions about this work :
;
; First, about the relevant Mathematics :
; 1)-A non-real Gaussian-Mersenne number can be written in the form : GM(p) = (1±i)^p - 1
; It is easy to prove that GM(p) can be a Gaussian prime only if p is prime, so we will only
; consider odd primes values of p, as for the Mersenne Prime Search.
; 2)-A non-real Gaussian integer G = x+iy is a Gaussian prime ssi its norm x^2+y^2 is a rational prime.
; 3)-The number M(p) = 4^p+1 with p odd has always the trivial factor 5, and can be written as :
; M(p) = (2^p + 2^((p+1)/2) + 1)(2^p - 2((p+1)/2) + 1)		(Aurifeuillian factorization)
; Moreover, M(p) can be written as M(p) = NG(p)*NF(p), 
; NG(p) beeing the norm of the Gaussian-Mersenne GM(p) =  (1±i)^p - 1
; NF(p) beeing the norm of the number : GF(p) = (1±i)^p + 1 (Gaussian-Fermat ?)
; Indeed, GF(p) beeing divisible by (2±i), its norm is always divisible by 5, but NF/5 may be prime, so, also GF/(2±i)!
; if p = 1 or 7 modulo 8, we have NG(p) = (2^p - 2((p+1)/2) + 1) and NF(p) = (2^p + 2^((p+1)/2) + 1)
; if p = 3 or 5 modulo 8, we have NG(p) = (2^p + 2((p+1)/2) + 1) and NF(p) = (2^p - 2^((p+1)/2) + 1)
;
; Now, I can tell what was to do in order to adapt the factor32 code for this task :
;
; First, about the sieve :
; A non-trivial factor of 4^p+1 must be of the form : 4*k*p+1, so it must be 1 or 5 modulo 8.
; So, the 16 "rem" values have been updated taking this in account. and the minimal factor increment is now 4*p.
; The "facdist" value remains unchanged, 120*p. Fortunately, that's all for the sieve!
;
; For the factoring code itself, I found at last that the easiest way was to modify slightly the presquaring code,
; and to set the exponent to 2*p+1, while skipping the last doubling of the remainder. (The exception is the 74/86 bit
; SSE2 code, for which the exponent is really set to 2*p). All versions of the code have been tested, and a special
; test mode has been implemented to do that.
;
; When a factor of 4^p+1 is found, the problem is that it may be a factor of NG(p) or one of NF(p) (exclusively,
; because NG - NP beeing a power of two, these numbers are co-prime), so, the program must be able to continue
; searching a factor until the sieve is done. I implemented this feature in all versions of the code.
;
; Noisy le Sec, March 30 2006
...
returnz	DD	OFFSET zx1
returna	DD	OFFSET ax1
returnb	DD	OFFSET bx1
continaddr	DD	2	; Area used to save the context after a success -JP-
continaddr2	DD	0
testaddr	DD	0
seax	DD	0
sebx	DD	0
...
seax2	DD	0
sesi2	DD	0
sfac32	DD	0	; To save the next 32 bit factor that should be tested after a success -JP-
...
cmpvalp		DD	0	; The value -(fac-1)/2 float for sqexit -JP-
...
reps		DD	0
p			DD	0	; Prime exponent being tested
twop		DD	0	; Multiples of p + p
fourp		DD	0	; Multiples of 4*p
last_primearray	DD	0	; Last address in primearray
;;rems		DD	1,7,17,23,31,41,47,49,71,73,79,89,97,103,113,119	;; George's values for Mersenne
rems		DD	1,13,17,29,37,41,49,53,61,73,77,89,97,101,109,113	;; New rems are 1 or 5 modulo 8 -JP-
QUARTER		DD	0.25
TWO			DD	2.0
FOUR		DD	4.0
TWO_TO_32	DD	0.0
...
; Save p (passed in _FACLSW), compute various constants and addresses
	...
	mov	twop, ecx
	add ecx, ecx		; Four times p
	mov fourp, ecx		; -JP-
	inc	ecx				; First 32bit factor to be tested -JP-
	mov	sfac32, ecx		; Save it for fac32 code -JP-
...
psq1:	inc	edx
	...
	jnz	short psq1
;	mov	ecx, p
	mov	ecx, twop		; The program will compute 2^twop modulo the factor -JP-
	inc	ecx				; The shifter works only on odd exponents... -JP-
	sub	eax, eax
...
idone2:	mov	initval0, eax		; Save bits 65-96 of initval
	mov	eax, _FACTEST
	cmp	eax, 16					; Is it normal mode? -JP-
	jl	short sversion			; Jump if yes -JP-
	cmp	eax, 70					; Is it 74 or 80 or 86 bits test mode? -JP-
	jg	testmode				; Jump if yes -JP-
	mov	eax, testaddr			; version to be tested...
	jmp	cp1						; For test...
...
;
; Try to find a 32-bit factor of 4**p + 1 -JP-
;

fac32:	mov	ecx, twop		; Exponent is 2*p -JP-
	inc	ecx					; Make exponent odd for the shifter... -JP-
s32lp:	add	ecx, ecx		; Shift until top bit on
	jns	short s32lp
	mov	eax, ecx			; Compute shifter and initial value. -JP-
	shl	eax, 5
	mov	shifter, eax
	shr	ecx, 27
	mov	eax, 1
...
; First trial factor is 4p + 1

	mov	ecx, sfac32			; Get current factor -JP-

; Factor is 1 or 5 mod 8 -JP-

; Square the number until we computed 2**(2*p) MOD factor

test32:	mov	ebx, shifter
	mov	eax, temp			; Initial value
	sub	edx, edx
	div	ecx					; modulo the factor
loop32:	mov	eax, edx		; square the value
	mul	eax
	div	ecx					; modulo factor
	add	ebx, ebx			; shift left and test the shifter
	jnc	short loop32		; current bit is zero
	jz	short exit32		; end of loop (note that, fortunately, last doubling is not done!) -JP-
	add	edx, edx			; double the remainder
	jc	short sub32			; take in account a possible overflow...
	cmp	edx, ecx			; adjust for remainder < factor
	jb	short loop32
sub32:	sub	edx, ecx
	jmp	short loop32
	
; If result = factor-1, then we found a divisor of 4**p + 1
...
win32:
	mov	eax, 2			; Return for ESC check -JP-
	mov	continaddr, eax	; Restore continaddr -JP-
	mov	eax, 1
	mov	_FACLSW, ecx	; Return the factor
	add	ecx, fourp		; -JP-
	jc	short over32	; Jump if 32 bit factoring is over. -JP-
	mov	sfac32, ecx		; Save next trial 32 bit factor
	jmp	done
...
;
; Try to find a 64-bit (or even more) factor of 4**p + 1
...
; Is this a request to test 32-bit factors?  If so, go to special code.
	...
	mov	eax, _FACTEST			; if test is > 15, it is a test run... -JP-
	cmp	eax, 16					; is it test mode? -JP-
	jl	fact264					; Jump if no. -JP-
	mov	edx, OFFSET ulp			; If test number < 60, test 486 code -JP-
	cmp	eax, 60
	jl	stestaddr
	mov	edx, OFFSET tlp60
	je	stestaddr
	mov	edx, OFFSET tlp62
	cmp	eax, 62
	je	stestaddr
	mov	edx, OFFSET tlp64
	cmp	eax, 64
	je	stestaddr
	mov	edx, OFFSET plp			; If 64 < test number <= 70, test Pentium pro code -JP-
	cmp	eax, 70
	jle	stestaddr
	

; This special code is used to test the 80 bit non-SSE2 and 74/86 bit SSE2 factoring codes -JP- 

	mov	ebx, _FACLSW			; ONLY FOR TESTS -JP-
	mov	eax, _FACHSW
	mov	ecx, _FACMSW			; We are only testing one given factor! -JP-
	mov	savefac0, eax
	mov	savefac1, ecx
	mov	savefac2, ebx
	call	presq
	wait
	sub	edi, edi		; Count of queued factors for SSE2 code -JP-
	...
	mov	eax, _FACTEST
	cmp	eax, 80				; Testing tlp80? -JP-
	je	zestit2				; Go to this test. -JP-
	cmp	eax, 74				; Testing tlp74? -JP-
	je	aestit2				; Go to this test. -JP-
	cmp	eax, 86				; Testing tlp86? -JP-
	je	bestit2				; Go to this test. -JP-
	
	fcompp					; pop two floating reg. -JP-
	mov	eax, 2				; Unsuccessful return -JP-
	mov	continaddr, eax
	jmp	done

stestaddr:
	mov	testaddr, edx		; Set address of code to be tested -JP-	
fact264:
	mov	eax, continaddr			; Is it continuing after a success ? -JP-
	cmp	eax, 2
	je	short begin64			; No -JP-
	push	eax					; Push return address -JP-
	cmp	eax, returnz			; Is it continuing 80 bits factoring ? -JP-
	je	short restf64			; Yes -JP-
	cmp	eax, OFFSET atest2
	je	short restf64			; Yes -JP-
	cmp	eax, returna
	je	short restf64			; Yes -JP-
	cmp	eax, OFFSET btest2
	je	short restf64			; Yes -JP-
	cmp	eax, returnb
	jne	short rest64			; No -JP-
restf64:						; Restore st(1) and st(0) -JP-
	fild	QWORD PTR savefac1	; Load savefac0 and savefac1
	fmul	TWO_TO_32
	mov	eax, savefac2		; Copy savefac2 for loading as a QWORD
	mov	faclow, eax
	...
	fild	QWORD PTR faclow	; Load savefac2
rest64:
	mov	eax, seax				; Restore the general registers context -JP-
	mov	ebx, sebx
	mov	ecx, secx
	mov	edx, sedx
	mov	esi, sesi
	mov	edi, sedi
	mov	ebp, sebp
	retn						; Continuing, go to relevant code
...
;	mov	fac2, twop		; Load twop for dividing
	mov	fac2, fourp		; Load fourp for dividing -JP-
	...
	div	fac2
	sub	fac2, edx		; Subtract remainder from fourp
...
; Return so caller can check for ESC
	...
	mov	eax, 2			; Return for ESC check
	mov	continaddr, eax	; Restore continaddr
	mov	_FACMSW, fac1
	jmp	done
...
fhiok:
	...
	sub	eax, ecx		; U - Sub shift count float exponent
	 ...
	 mov	cmpvalp, eax			; Store -(fac-1)/2  float -JP-
	 xor	eax, 80000000h			; change sign to compute (fac-1)/2 -JP-

	; Compute the floating value -(fac+1)/2 -JP- .  Well, we don't have
...
;
; If result = -1 mod factor, then we found a divisor of 4**p + 1. -JP-
; As a quick test, compare the top 32 bits of the float using integer
; instructions.  This will quickly eliminate most values.
;

					; rem, fachi, rem, 1/fac
	fadd	cmpvalp		; Compute rem - (fac-1)/2 -JP-
	fst	temp			; Store the corrected remainder
	mov	eax, temp		; U - Load the corrected remainder
	...
	; Add one, subtract factor/2 and test if number equals factor/2 or -factor/2.
	
smaybe:	;fadd	st, st			; do not double ; trash, trash, rem, trash -JP-
	fcompp				; Pop two trash values ; rem, trash -JP-
;	fsub	ONE			; Subtract one
	fadd	ONE			; Add one ; rem+1, trash -JP-
	fild	QWORD PTR savefac2	; factor, result, trash -JP-
	fmul	HALF		; factor/2, result, trash -JP-
	fxch	st(1)		; result, factor/2, trash -JP-
	fsub	st(0), st(1) ; result, factor/2, trash -JP-
	fabs				; Take absolute value of result
	fcompp				; Does result = factor/2 ; trash -JP-
	fstsw	ax			; Copy comparison results
	fcomp				; Pop last trash value -JP-
...
winner:	mov	eax, savefac1		; Load MSW
	...
	pop	ecx			; Restore sieve testing register
	pop	eax			; Restore sieve testing register
	mov	seax, eax	; save all the context. -JP-
	mov	sebx, ebx
	mov secx, ecx
	mov sedx, edx
	mov sesi, esi
	mov	sedi, edi
	mov	sebp, ebp
	pop	eax			; pop return address -JP-
	mov	continaddr, eax	; -JP-
...
; rem2 being -1 modulo their factor. To do that, I test if |2*rem1+2-fac1| - fac1 is zero,
; and/or |rem2+1-fac2/2| - fac2/2 is zero. -JP-
...
;	fsub	ONE			;12 rem1 = rem1 - 1
	...
	fxch	st(2)			; fac2,rem2,rem1
	  fmul	HALF			;19   fac2 = fac2/2
					; fac2/2,rem2,rem1
	  fxch	st(1)			; rem2,fac2/2,rem1
;	  fsub	HALF			;20   rem2 = rem2 - 1/2
	fadd	ONE			; rem2 = rem2 + 1 -JP-
	fsub	st(0), st(1)	; rem2 -= fac2/2
...
vtest2:
	...
	cmp	ecx, 00000000h		;32U Compare remainder to 0.0
	je	short vin2		;32V
	mov	ecx, continaddr2	; Restore the return address
	push ecx
	retn				;33UV - Test next factor from sieve

vin1: fstp	temp			;29-30 Save rem2 for comparing
	fld	FACHI			; Load MSW
	fadd	FACLO			; Load LSW
	fistp	QWORD PTR savefac2	; Save as an integer
	mov	eax, savefac2		; Load LSW
	...
	mov	sedi, edi
	mov	sebp, ebp
	mov	eax, OFFSET vtest2	; set return address for second test -JP-
	mov	continaddr, eax	; JP
	pop	eax			; pop return address for second factor not found -JP-
	mov	continaddr2, eax	; -JP-
	mov	eax, 1			; Factor found!!! Return TRUE
...
;	fsub	ONE			;12 rem1 = rem1 - 1
	...
	fxch	st(2)			; fac2,rem2,rem1
	  ...
	  fxch	st(1)			; rem2,fac2/2,rem1
;	  fsub	HALF			;20   rem2 = rem2 - 1/2
	fadd	ONE				;20   rem2 = rem2 + 1 -JP-
	fsub	st(0), st(1)	; rem2 -= fac2/2 -JP-
	  fxch	st(2)			; rem1,fac2/2,rem2
	fsub	FACLO			;21 rem1 -= FACLO
	fxch	st(3)			; 1/fac1,fac2/2,rem2,rem1
	fcomp	st			;22 pop 1/fac value
...
wtest2:
	...
	cmp	ecx, 00000000h		;32U Compare remainder to 0.0
	je	win2		;32V
	mov	ecx, continaddr2	; Restore the return address -JP-
	push ecx
	retn				;33UV - Test next factor from sieve

win1: fstp	temp			;29-30 Save rem2 for comparing
	fld	FACHI			; Load MSW
	fadd	FACLO			; Load LSW
	fsub	ONE			; Make it fit in 63 bits
	fmul	HALF
	...
	mov	sedi, edi
	mov	sebp, ebp
	mov	eax, OFFSET wtest2	; set return address -JP-
	mov	continaddr, eax	; -JP-
	pop	eax			; pop return address for second factor not found -JP-
	mov	continaddr2, eax	; -JP-
	mov	eax, 1			; Factor found!!! Return TRUE
...
;
; If result = -1 mod factor, then we found a divisor of 4**p + 1
;

uqexit:	fstp	quotient2		; Pop 1 / factor from FPU
;	add	esi, esi		; Don't double the remainder -JP-
	pop	eax			; Restore sieve testing register
	pop	edx
	add	esi, 1			; Add one to the remainder -JP-
	jnz	short upos2		; Remainder could not be -1 -JP-
	adc	edi, 0			; Propogate carry -JP-

	; Handle a possibly -1 remainder, see if value is 0 -JP-
	
	jnz	short upos		; Not zero, test for factor-1 -JP-
	jmp	short uwin		; We have found a factor if value is zero -JP-
...
;	retn				; Test next factor from sieve
	...
	; Handle a possibly positive remainder, subtract factor and see if value is 0 -JP-
	
upos2:	adc	edi, 0		; Propogate carry -JP-
upos:	sub	esi, fac2		; Subtract factor
	sbb	edi, fac1
	jz	short umaybe
...
;	push	esi			; Save sieve testing registers
;	push	eax
	mov	sesi2, esi			; Save sieve testing registers -JP-
	mov	seax2, eax			; But we don't use the stack for that! -JP-
	mov	ebp, shifter		; Load shifter
...
; If result = -1 mod factor, then we found a divisor of 4**p + 1.
; The successful  remainder will equal factor-1 or -1. -JP-
;
...
pret:
;	pop	eax			; Restore sieve testing registers
;	pop	esi
	mov	eax, seax2		; Restore sieve testing registers -JP-
	mov	esi, sesi2
...
; If abs(2*rem+2-fac) == fac, we have found a factor of 4**p+1
...
;	fsub	ONE			; Subtract one
	...
	fxch	st(1)		; 2*rem+2, fac -JP-
	fsub	st(0), st(1) ; 2*rem+2-fac, fac -JP-
	fabs				; Take the absolute value
	fcompp				; Compare result to factor
	fstsw	ax			; Copy comparison results
	and	eax, 4000h		; Isolate C3 bit
...
zestit2:	fldz			; Only for tests -JP-
	fadd	st, st(1)
	fld	st(0)
...
;
	...
	jmp	zqloop
;
; Don't multiply remainder by two one last time (ignore the last carry out of shifter)
; If result = -1 mod factor, then we found a divisor of 4**p + 1 -JP-
;

zqexit:	fstp	quotient2		; Pop 1 / factor from FPU
	add	esi, 1		; Add one to the remainder -JP-
	jnz	short zt1	; Jump if result is not zero -JP-
	adc	ecx, 0
	jnz short zt2	; Jump if result is not zero -JP-
	adc	ebx, 0
	jz	short zwin	; Factor found ! -JP-
	jmp	short zt3	; Jump if result is not zero -JP-
zt1:
	adc	ecx, 0
zt2:	
	adc	ebx, 0
...
zloser:
	mov	eax, _FACTEST
	cmp	eax, 80		; is it test mode? -JP-
	jne	short zloser2	; Continue sieving if no -JP-
	pop	esi			; Restore sieve testing register
	pop	eax
	mov	eax, 2
	mov	continaddr, eax	; -JP-
...
zwin:	mov	eax, fachigh		; Factor found!!! Return it
	...
	mov	eax, faclow
	mov	_FACLSW, eax
	pop	esi			; Restore sieve testing register -JP-
	pop	eax
	mov	seax, eax	; save all the context. -JP-
	mov	sebx, ebx
	mov secx, ecx
	...
	mov	sedi, edi
	mov	sebp, ebp
	mov	eax, returnz	; Return address -JP-
	mov	continaddr, eax	; -JP-
...
; Compute factor - 1 for comparing against when loop is done -JP-

	mov	ebx, savefac1		; Compute again the factor as an integer
	mov	ecx, savefac0
	mov	ebp, faclow
	add	ebx, facmid
	adc	ecx, 0				; Subtract one -JP-
	sub	ebp, 1
	sbb	ebx, 0
	sbb	ecx, 0
	shld	ecx, ebx, 4
...
; Do other initialization work
	...
	jmp	asse2go		; Go to test the queued factors

aestit2:	fldz		; Initialize for test mode -JP-
	fadd	st, st(1)
	fld	st(0)
	fistp	QWORD PTR faclow	; Save lower bits of factor
	fadd	st, st(2)		; We now have the factor to test
...
;	btr	eax, edx		; Clear the sieve bit
	inc	edi			; Increment count of queued factors
	cmp	edi, 2			; Test count of queued factors
	jne	aestit2			; Get the same factor again 
	sub	edi, edi		; Reset count of queued factors
...
;
; If result = factor - 1, then we found a divisor of 4**p + 1 -JP-
;

	pcmpeqd	xmm2, XMM_COMPARE_VAL3	; See if remainder is factor - 1 -JP-
...
atest2:
	...
	mov	ecx, _FACTEST
	cmp	ecx, 74			; Is it test mode? -JP-
	jne	ax1				; Test next factor from sieve if no. -JP-
	mov	eax, 2			; Unsuccessful return... -JP-
	mov	continaddr, eax
	jmp	done
awin1:	mov	eax, OFFSET atest2	; Return address -JP-
	mov	continaddr, eax	; -JP-
	mov	eax, XMM_F3		; Factor found!!! Return it
	mov	ebx, XMM_F2
	mov	ecx, XMM_F1
	jmp	short awin3
awin2:	mov	eax, returna	; Return address -JP-
	mov	continaddr, eax	; -JP-
	mov	eax, XMM_F3+8		; Factor found!!! Return it
	mov	ebx, XMM_F2+8
	mov	ecx, XMM_F1+8
awin3:	shl	eax, 2
	shrd	eax, ebx, 2
...
; Do other initialization work
	...
	sub	edi, edi		; Reset count of queued factors
	jmp	bsse2go			; Go to test the factors -JP-

bestit2:	fldz		; Initialize for test mode -JP-
	fadd	st, st(1)
	fld	st(0)
	fistp	QWORD PTR faclow	; Save lower bits of factor
	fadd	st, st(2)		; We now have the factor to test
...
; Do other initialization work
	...
	inc	edi			; Increment count of queued factors
	cmp	edi, 2			; Test count of queued factors
	jne	bestit2			; Get the same factor again. -JP-
	sub	edi, edi		; Reset count of queued factors
...
;
	...
	pmovmskb ecx, xmm2
	cmp	cl, 0FFh		; See if we matched
	je	short bwin1		; Yes! Factor found -JP-
btest2:
	cmp	ch, 0FFh		; See if we matched
	je	short bwin2		; Yes! Factor found -JP-
	mov	ecx, _FACTEST
	cmp	ecx, 86			; Is it test mode? -JP-
	jne	bx1				; Test next factor from sieve if no. -JP-
	mov	eax, 2			; Unsuccessful return... -JP-
	mov	continaddr, eax
	jmp	done
bwin1:	mov	eax, OFFSET btest2	; Return address -JP-
	mov	continaddr, eax	; -JP-
	mov	eax, XMM_F3		; Factor found!!! Return it
	mov	ebx, XMM_F2
	mov	ecx, XMM_F1
	jmp awin3
bwin2:	mov	eax, returnb	; Return address -JP-
	mov	continaddr, eax	; -JP-
	mov	eax, XMM_F3+8		; Factor found!!! Return it
	mov	ebx, XMM_F2+8
	mov	ecx, XMM_F1+8
