...
int gmodi (	/* Returns g%i as an integer */ 
	...
	value = 0;
	for (j=0; j<size; j++) {
	    value += (ULONG)((__int64)(g->n[j]%i)*wordweight)%i;
	    if (value >= i) value -= i;
...
ULONG twopownmodm (ULONG n, ULONG m, ULONG *order, ULONG *nmodorder) {
	ULONG tpnmodm, tp, i, work, mask = 1<<31;
...
/*	    for (i=1, tp=1; i<=*nmodorder; i++) {
	    ...
	    tpnmodm = (ULONG)ltp;	// 2^n modulo m == 2^(n%order) modulo m
	}
	return tpnmodm;
}
...
int gen_v1(giant k, ULONG n, int general, int eps2, int debug) {
	long sign, jNd, jNa, v;
	ULONG kmod3, rawd, d, dred, kmodd, tpnmd, i, orderd, Nmodd;
	ULONG X, Y, aplus, aminus, b, rplus, rminus;
...
int genLucasBaseQ(giant N, ULONG D) {
	ULONG NmodD, dred, Nmod8;
	int jNp, chgsign;
...
int isLucasBaseQ(giant N, ULONG D, int sign) {
	ULONG NmodD, dred, Nmod8;
	int jNp, chgsign;
...
int genLucasBaseP(giant N, ULONG P) {
	ULONG NmodD, D, dred, Nmod8;
	int jNp, chgsign;
...
long generalLucasBase(giant N, ULONG *P, ULONG *Q) {
	ULONG NmodD, D, dred, Nmod8, NmodPQD, gcdNPQD;
	int jNp, chgsign;
...
//  if D = 2^s*t, (N / D) = (N / 2)^s * (N / t) = (N / 2)^s * (Nmodt / t)
	...
	for (*P; (*P)*(*P)<=2147483647; (*P)++) {
		for ((*Q)=2; 4*(*Q)<(*P)*(*P); (*Q)++) {
			D = (*P)*(*P)-4*(*Q);
			if ((ULONG)(floor(sqrt ((double)D)) * floor(sqrt ((double)D))) == D) {
				continue;
			}
			dred = D;
			chgsign = 1;
